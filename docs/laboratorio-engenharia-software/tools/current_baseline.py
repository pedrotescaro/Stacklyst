...
def apply_current_baseline(ns):
    ...
    by_activity['AN02'].update(
        flow=['Selecionar convite direto ou arena.', 'No convite, aguardar até 72 horas; rejeição ou silêncio não gera punição, e a expiração publica na arena somente se o desafiante autorizar.', 'Na arena, ocupar um duelo PENDING ou criar um para aguardar oponente por até 24 horas.', 'Validar participante, linguagem suportada e prazo de duas horas.', 'Executar os testes do código no servidor e registrar DuelSubmission.', 'Resolver por pontuação quando houver resultados ou ao atingir o prazo; ausência de ambos os envios encerra sem vencedor.', 'Encaminhar empate técnico para REVIEW_PENDING e parecer humano autorizado.', 'Fechar o duelo, conceder 50 XP ao vencedor e contar abandono somente de quem aceitou/entrou e não enviou solução.'],
        evidence='src/app/api/duels/route.ts; src/services/duel.service.ts; src/lib/duels/{invitations,judge,resolution,resolution-policy,lifecycle}.ts')
    ...
    updates = {
        ...
        'RF017': dict(description='Entrar em duelo disponível ou aguardar oponente na arena.', processing='Atualização condicional ocupa duelo PENDING compatível; se não houver, cria pendência. Convite expirado também pode entrar na arena quando o desafiante autorizou.', details='Implementado: src/app/api/duels/route.ts; src/lib/duels/invitations.ts; src/lib/duels/lifecycle.ts.', constraints='A arena aguarda por 24 horas; convite direto aguarda por 72 horas.'),
    ...
    uc['UC008'].update(name='Convidar para duelo', secondary='Oponente', description='Enviar convite direto ou por busca de oponente.', preconditions='Conta autenticada, oponente elegível e cooldown cumprido.', postconditions='Convite pendente por 72 horas; o duelo só inicia após aceite.', inputs='Linguagem, oponente opcional e autorização opcional de publicação.', outputs='Convite, prazo e estado.', flow=['Escolher linguagem e oponente ou solicitar busca.', 'Validar sessão, disponibilidade e cooldown por abandono.', 'Selecionar oponente elegível quando necessário.', 'Registrar convite pendente e informar o destinatário.', 'Aguardar aceite, rejeição ou expiração automática.'], alternatives=['A1 — Convite expirado: publicar na arena somente se autorizado; caso contrário, encerrar.', 'A2 — Rejeição ou silêncio não reduz XP e não gera cooldown.', 'A3 — Cooldown por três abandonos impede novo convite.'])
