...
ACTIVITIES = [
    ...
    },
    {
        ...
        "flow": ["O desafiante solicita duelo direto ou busca automática.", "O Stacklyst verifica se o usuário pode duelar e busca um oponente compatível.", "Se nenhum oponente for localizado, informa indisponibilidade; se houver, envia um convite válido por 72 horas.", "O oponente analisa e aceita ou rejeita; rejeição e silêncio não geram punição.", "Na expiração, o sistema publica na arena somente quando o desafiante autorizou.", "Com o aceite, o sistema cria o desafio e ambos enviam soluções em até duas horas.", "O sistema recebe as soluções e decide se a avaliação automática é suficiente ou se deve encaminhar a um avaliador.", "A avaliação é registrada, o duelo é fechado e os participantes consultam o resultado."],
...
RF_GROUPS = [
    ...
    ("Grupo 4 — Duelos", [
        ...
        make_rf("RF018", "Aceitar ou rejeitar oponente", "Alta", "Permitir decisão sobre convite dentro do prazo.", "Oponente", "RN008, RN009", "Implementado", "src/app/api/duels/respond; DuelService.respondDuelRequest", "Convite e ação ACCEPT/REJECT.", "Verificar destinatário, status e expiração.", "Duelo criado ou rejeição sem punição registrada.", "Apenas o destinatário responde; convite já processado não pode ser reutilizado."),
...
BUSINESS_RULES = [
    ...
    ("RN008", "Prazo do convite", "Convite direto expira em 72 horas; publicação na arena por 24 horas depende de autorização do desafiante.", "Implementado", "DUEL_REQUEST_TIMEOUT_SECONDS; PUBLIC_DUEL_MATCH_WINDOW_SECONDS"),
    ("RN009", "Rejeição e cooldown", "Rejeitar ou ignorar não pune. Três abandonos consecutivos após aceite/entrada aplicam cooldown de 24 horas.", "Implementado", "MAX_CONSECUTIVE_DUEL_ABANDONS; DUEL_ABANDON_COOLDOWN_HOURS"),
    ("RN010", "Tempo do desafio", "Todo duelo ativo possui prazo de 7.200 segundos (duas horas).", "Implementado", "DUEL_TIME_LIMIT_SECONDS"),
...
def build_use_cases() -> None:
    ...
    for text in [
        "Validar regras atuais: convite de 72 h, arena de 24 h, duelo de 2 h e cooldown de 24 h após 3 abandonos.",
